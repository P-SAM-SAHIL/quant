...
def apply_spinquant(model, seed, rotation_path=None):
    ...
    with torch.no_grad():
        ...
        # Undo R1 before the final norm.  Keeping this operation explicit
        # avoids relying on RMSNorm's finite-precision rotation invariance.
...
def main():
    ...
    for condition in conditions:
        ...
        if condition == "rtn_w4":
            ...
            # The model is BF16 and every rotated projection is rounded back
            # to BF16.  A 0.999 threshold incorrectly rejects valid rotations
            # after many decoder layers; 0.99 still catches a wrong transpose
            # or a corrupted rotation checkpoint.
            if rotation_cosine < 0.99:
                raise RuntimeError(f"rotation is not numerically valid (cosine={rotation_cosine:.6f})")
            replace_projections(model, args.bits, args.groupsize, not args.asym)
